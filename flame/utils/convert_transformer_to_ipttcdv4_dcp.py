...
def convert_transformer_to_ipttcdv4_dcp(
    model: str,
    config: str,
    checkpoint: Path,
    ...
    # The actual target architecture is determined by the provided config (model_type=ipttcdv4).
    convert_transformer_to_ipttcdv3_dcp(
        model=model,
        config=config,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Convert a matching FLA transformer HF checkpoint to an IPTTCDv4 DCP checkpoint."
    )
    parser.add_argument("--model", type=str, required=True, help="HF source model path or repo id")
    parser.add_argument("--config", type=str, required=True, help="Target IPTTCDv4 config path")
    parser.add_argument(
        "--tokenizer",
        type=str,
        default=None,
    ...
    convert_transformer_to_ipttcdv4_dcp(
        model=args.model,
        config=args.config,
