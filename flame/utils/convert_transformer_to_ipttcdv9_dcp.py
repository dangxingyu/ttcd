...
def convert_transformer_to_ipttcdv9_dcp(
    model: str,
    config: str,
    checkpoint: Path,
    ...
    # The actual target architecture is determined by the provided config (model_type=ipttcdv9).
    convert_transformer_to_ipttcdv3_dcp(
        model=model,
        config=config,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Convert a matching FLA transformer HF checkpoint to an IPTTCDv9 DCP checkpoint."
    )
    parser.add_argument("--model", type=str, required=True, help="HF source model path or repo id")
    parser.add_argument("--config", type=str, required=True, help="Target IPTTCDv9 config path")
    parser.add_argument(
        "--tokenizer",
        type=str,
        default=None,
    ...
    convert_transformer_to_ipttcdv9_dcp(
        model=args.model,
        config=args.config,
