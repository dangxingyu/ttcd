...
from custom_components.optimizer import build_optimizers
...
def _log_and_snapshot_model_config(job_config: JobConfig) -> None:
    config_path = os.path.abspath(job_config.model.config)
    logger.info(f"Resolved model config path: {config_path}")

    if not os.path.isfile(config_path):
        logger.warning(f"Model config path does not exist on disk: {config_path}")
        return

    with open(config_path, "r", encoding="utf-8") as f:
        raw_config = f.read()

    config_sha256 = hashlib.sha256(raw_config.encode("utf-8")).hexdigest()
    logger.info(f"Model config sha256: {config_sha256}")
    logger.info(f"Raw model config JSON:\n{raw_config}")

    if torch.distributed.is_initialized() and torch.distributed.get_rank() != 0:
        return

    dump_folder = os.path.abspath(job_config.job.dump_folder)
    os.makedirs(dump_folder, exist_ok=True)

    snapshot_path = os.path.join(dump_folder, "resolved_model_config.json")
    meta_path = os.path.join(dump_folder, "resolved_model_config.meta.json")
    ...
    logger.info(f"Saved resolved model config snapshot to {snapshot_path}")
    logger.info(f"Saved resolved model config metadata to {meta_path}")
...
# ============================================================
# To trigger an immediate checkpoint save during training:
#   touch /tmp/flame_save_checkpoint
# Or send SIGUSR1 to rank 0 process:
#   kill -SIGUSR1 <pid>
# ============================================================
_CHECKPOINT_TRIGGER_FILE = "/tmp/flame_save_checkpoint"
_force_save_checkpoint = False


def _sigusr1_handler(signum, frame):
    """Handle SIGUSR1 signal to trigger immediate checkpoint save."""
    global _force_save_checkpoint
    _force_save_checkpoint = True
    logger.info("[SIGNAL] Received SIGUSR1 - will save checkpoint at next step")


def _check_save_trigger() -> bool:
    """Check if an external trigger requests immediate checkpoint save.
    Returns True if a trigger is detected (signal or trigger file).
    """
    global _force_save_checkpoint

    # Check signal flag
    if _force_save_checkpoint:
        _force_save_checkpoint = False
        logger.info("[TRIGGER] Checkpoint save triggered by SIGUSR1 signal")
        return True

    # Check trigger file
    if os.path.exists(_CHECKPOINT_TRIGGER_FILE):
        try:
            os.remove(_CHECKPOINT_TRIGGER_FILE)
        except OSError:
            pass
        logger.info("[TRIGGER] Checkpoint save triggered by trigger file")
        return True
...
def main(job_config: JobConfig):
    # Register SIGUSR1 handler for checkpoint triggering
    ...
    dataset = build_dataset(
        ...
        shuffle_buffer_size=job_config.training.shuffle_buffer_size,
        parquet_prefetch_limit=job_config.training.parquet_prefetch_limit,
        parquet_range_size_mb=job_config.training.parquet_range_size_mb,
        dp_degree=dp_degree,
    ...
    _log_and_snapshot_model_config(job_config)
    _ensure_custom_model_registration(job_config.model.config)
    ...
    # Handle absolute lr_min: convert to relative ratio if specified
    if job_config.lr_scheduler.lr_min_absolute is not None:
        base_lr = job_config.optimizer.lr
        lr_min_absolute = job_config.lr_scheduler.lr_min_absolute
        if lr_min_absolute >= base_lr:
            logger.warning(
                f"lr_min_absolute ({lr_min_absolute}) >= optimizer.lr ({base_lr}). "
                f"Setting lr_min to 1.0 (no decay)."
            )
            job_config.lr_scheduler.lr_min = 1.0
        else:
            lr_min_ratio = lr_min_absolute / base_lr
            logger.info(
                f"Using absolute lr_min: {lr_min_absolute:.2e} "
                f"(relative ratio: {lr_min_ratio:.6f} with base_lr={base_lr:.2e})"
            )
            job_config.lr_scheduler.lr_min = lr_min_ratio

    # Handle immediate annealing: override training.steps and decay_ratio
    # so that WSD decay starts exactly at anneal_begin_step
    anneal_begin_step = getattr(job_config.lr_scheduler, 'anneal_begin_step', None)
    if anneal_begin_step is not None:
        decay_ratio = job_config.lr_scheduler.decay_ratio
        if decay_ratio is None or decay_ratio <= 0:
            raise ValueError(
                f"anneal_begin_step={anneal_begin_step} requires a positive decay_ratio, "
                f"got decay_ratio={decay_ratio}"
            )
        anneal_steps = round(anneal_begin_step * decay_ratio)
        new_total_steps = anneal_begin_step + anneal_steps
        new_decay_ratio = anneal_steps / new_total_steps
        old_total_steps = job_config.training.steps
        logger.info(
            f"[Anneal] Immediate annealing from step {anneal_begin_step}:\n"
            f"  decay_ratio={decay_ratio} → anneal_steps={anneal_steps}\n"
            f"  training.steps: {old_total_steps} → {new_total_steps}\n"
            f"  lr_scheduler.decay_ratio: {decay_ratio} → {new_decay_ratio:.6f}\n"
            f"  Decay phase: step {anneal_begin_step} → {new_total_steps}"
        )
        job_config.training.steps = new_total_steps
        job_config.lr_scheduler.decay_ratio = new_decay_ratio
    ...
    with (
        ...
        while train_state.step < job_config.training.steps:
            ...
            losses = []
            aux_losses = []  # Track auxiliary losses for MoE models
            # do gradient accumulation if enabled
            for _accum_idx in range(job_config.training.gradient_accumulation_steps):
                # get batch
                ...
                if parallel_dims.pp_enabled:
                    ...
                    with train_context(optional_context_parallel_ctx):
                        ...
                        # Extract aux_loss if available (for MoE models)
                        aux_loss = getattr(output, 'aux_loss', None)
                        if aux_loss is not None:
                            aux_losses.append(aux_loss.detach() / job_config.training.gradient_accumulation_steps)

                        # Save last microbatch logits/labels for per-position loss (only at log steps)
                        if metric_logger.should_log(train_state.step) and output.logits is not None:
                            _last_logits = output.logits.detach()
                            _last_labels = labels.detach()
            ...
            # TODO (alex): make it a after training step hook
            if hasattr(model.config, 'bias_update_speed') and model.config.bias_update_speed > 0:
                for m in model_parts:
                    with torch.no_grad():
                        m.update_expert_bias()
            # Loss-Free expert bias update for moe_transformer (sonicmoe)
            if getattr(getattr(model_parts[0], 'config', None), 'loss_free', False):
                for m in model_parts:
                    with torch.no_grad():
            ...
            if metric_logger.should_log(train_state.step):
                ...
                # Add aux_loss metrics for MoE models (moe_transformer, moe_loop_transformer)
                if len(aux_losses) > 0:
                    # Sum accumulated aux_losses
                    ...
                    if (
                        ...
                        global_avg_aux_loss = total_aux_loss.item()

                    # Get aux_loss_coef from model config if available
                    aux_loss_coef = getattr(model_parts[0].config, 'aux_loss_coef', 0.01)

                    # Calculate pure LM loss = total_loss - aux_loss_coef * aux_loss
                    if isinstance(global_avg_loss, torch.Tensor):
                        lm_loss = global_avg_loss.item() - aux_loss_coef * (global_avg_aux_loss.item() if isinstance(global_avg_aux_loss, torch.Tensor) else global_avg_aux_loss)
                    else:
                        lm_loss = global_avg_loss - aux_loss_coef * (global_avg_aux_loss.item() if isinstance(global_avg_aux_loss, torch.Tensor) else global_avg_aux_loss)

                    extra_metrics_dict["loss/lm_loss"] = lm_loss
                    extra_metrics_dict["loss/aux_loss"] = global_avg_aux_loss.item() if isinstance(global_avg_aux_loss, torch.Tensor) else global_avg_aux_loss

                # Add ParScale metrics if available (cosine similarity + aggregation entropy)
                for m in model_parts:
                    if hasattr(m, 'get_parscale_cosine_similarities'):
                        parscale_metrics = m.get_parscale_cosine_similarities()
                        extra_metrics_dict.update(parscale_metrics)
                        break

                # Add IPTTCD TTT norm metrics if available
                for m in model_parts:
                    if hasattr(m, 'get_ttt_norm_metrics'):
                        ttt_norm_metrics = m.get_ttt_norm_metrics()
                        if ttt_norm_metrics:
                            extra_metrics_dict.update(ttt_norm_metrics)
                        break

                # Per-position loss at specific context lengths (2k, 4k, 8k, 16k)
                if '_last_logits' in dir() and _last_logits is not None:
                    try:
                        _seq_len = _last_logits.shape[1]
                        _vocab_size = _last_logits.shape[-1]
                        _shifted_labels = torch.cat(
                            (_last_labels[..., 1:],
                             torch.full_like(_last_labels[:, :1], -100)),
                            dim=1,
                        )
                        _per_token_loss = torch.nn.functional.cross_entropy(
                            _last_logits.reshape(-1, _vocab_size),
                            _shifted_labels.reshape(-1),
                            reduction='none',
                            ignore_index=-100,
                        ).reshape(_last_labels.shape[0], _seq_len)
                        ...
                        for _name, _start, _end in _pos_boundaries:
                            if _start < _seq_len:
                                _actual_end = min(_end, _seq_len)
                                _slice = _per_token_loss[:, _start:_actual_end]
                                _mask = _shifted_labels[:, _start:_actual_end] != -100
                                if _mask.any():
                                    _pos_loss = _slice[_mask].mean().item()
                                    extra_metrics_dict[f"loss_by_pos/{_name}"] = _pos_loss
                        del _last_logits, _last_labels
                    except Exception:
                        pass
                ...
                metric_logger.log(
                    ...
                    global_avg_loss,
                    global_max_loss,
                    extra_metrics=extra_metrics_dict,
                )

                # Build log message with optional aux_loss info
                log_msg = f"{color.blue}lr: {last_lr:.4e} gnorm: {grad_norm:5.2f}"
                if "loss/lm_loss" in extra_metrics_dict and "loss/aux_loss" in extra_metrics_dict:
                    log_msg += f" {color.green}lm_loss: {extra_metrics_dict['loss/lm_loss']:.4f} aux_loss: {extra_metrics_dict['loss/aux_loss']:.4f}"
                log_msg += f" {color.magenta}[{str(train_state.elapsed).split('.')[0]:>8}<{str(eta).split('.')[0]:>8}]{color.reset}"
                logger.info(log_msg)

            # Check for external save trigger (file or signal)
            force_save = (train_state.step == job_config.training.steps) or _check_save_trigger()
            checkpoint.save(train_state.step, force=force_save)
