...
def build_dataset(
    ...
    parquet_prefetch_limit: int = 1,
    parquet_range_size_mb: int = 32,
    ...
    dataset_path = dataset  # Save original path before reassignment

    # `mds:/path[?proc=...&domain=...]` dispatch — pre-tokenized MDS shards
    # (e.g. princeton-nlp/prolong-data-64K). Each MDS sample is already the
    # target seq_len, so flame's tokenize/repack path is bypassed entirely.
    if dataset.startswith("mds:"):
        from flame.data_mds import build_mds_dataset
        # seq_len is read from URI params if present, else the caller passes
        # via training.seq_len; we don't have access to it here, so let the
        # MDS reader default and rely on the URI ?seq_len=... override.
        return build_mds_dataset(dataset, seq_len=65536, seed=seed or 42)

    # Configure Parquet prefetching and buffering for streaming mode
    fragment_scan_options = pyarrow.dataset.ParquetFragmentScanOptions(
        cache_options=pyarrow.CacheOptions(
            prefetch_limit=parquet_prefetch_limit,
            range_size_limit=parquet_range_size_mb << 20,  # Convert MiB to bytes
        ),
    )
    logger.info(
        f"Parquet streaming config: prefetch_limit={parquet_prefetch_limit}, "
        f"range_size={parquet_range_size_mb}MiB"
    )
    ...
    if len(dataset.split(',')) == 1:
        ...
        if not streaming:
            ...
            if min_num_shards is not None and dataset.num_shards < min_num_shards:
                ...
                if seed is not None:
                    dataset = shuffle(dataset, seed=seed, buffer_size=shuffle_buffer_size)
    else:
        ...
        for i, prob in enumerate(data_probs):
            ...
            if not streaming:
                ...
                if min_num_shards is not None and subset.num_shards < min_num_shards:
                    ...
                    if seed is not None:
                        subset = shuffle(subset, seed=seed, buffer_size=max(128, shuffle_buffer_size // len(datasets)))
...
def build_dataloader(
    ...
    # Pre-tokenized datasets (e.g. our MDSPretokenizedDataset over ProLong)
    # already yield {'input_ids': tensor} dicts — skip the
    # OnlineTokenizedIterableDataset wrap that calls .shard() and
    # tokenize() (DP sharding is handled inside the MDS reader by
    # mosaicml-streaming).
    from flame.data_mds import MDSPretokenizedDataset
    if not isinstance(dataset, MDSPretokenizedDataset):
