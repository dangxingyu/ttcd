...
class JobConfig:
    ...
    def __init__(self):
        ...
        # TODO (alex): fine grained lr for different optimizers
        # TODO (alex): warmup for muon momentum
        self.parser.add_argument(
            "--optimizer.enable_muon",
            action="store_true",
            help="Whether to apply muon for matrix",
        )
        self.parser.add_argument(
            "--optimizer.muon_momentum", type=float, default=0.95, help="Momentum to use in muon"
        )
        self.parser.add_argument(
            "--optimizer.use_distributed_muon",
            action="store_true",
            help="Use distributed muon (allgather-based) instead of parallel muon (all2all-based). "
                 "More stable for multi-node training at the cost of redundant computation.",
        )
        ...
        self.parser.add_argument(
            ...
            default=0.0,
            help="""
            Min lr ratio for lr scheduler (relative value).
            ...
            Note: If `lr_min_absolute` is set, this parameter is ignored.
            """,
        )
        self.parser.add_argument(
            "--lr_scheduler.lr_min_absolute",
            type=float,
            default=None,
            help="""
            Absolute minimum learning rate value.

            If provided, the learning rate will decay to this absolute value instead of
            using the relative `lr_min` ratio. This takes precedence over `lr_min`.

            Example: --lr_scheduler.lr_min_absolute 1e-5 will decay LR to exactly 1e-5.
            """,
        )
        self.parser.add_argument(
            "--lr_scheduler.anneal_begin_step",
            type=int,
            default=None,
            help="""
            Trigger immediate annealing (WSD decay phase) starting at the specified step.

            When set, this overrides `training.steps` so that the WSD decay phase
            begins exactly at `anneal_begin_step`. The annealing duration is
            `round(anneal_begin_step * decay_ratio)` steps.

            New total_steps = anneal_begin_step + round(anneal_begin_step * decay_ratio).

            This is useful for WSD cooldown experiments where you want to branch
            off from a running training and immediately start annealing.

            Example: --lr_scheduler.anneal_begin_step 106000 --lr_scheduler.decay_ratio 0.1
            will start decay at step 106000 and anneal for 10600 steps (total 116600).
            """,
        )
        ...
        self.parser.add_argument(
            ...
            type=int,
            default=1024,
            help="Buffer size for shuffle in streaming mode. Larger buffer provides better randomness but uses more memory.",
        )
        self.parser.add_argument(
            "--training.parquet_prefetch_limit",
            type=int,
            default=1,
            help="Number of Parquet chunks to prefetch in background (streaming mode). Higher values use more memory but reduce I/O stalls.",
        )
        self.parser.add_argument(
            "--training.parquet_range_size_mb",
            type=int,
            default=32,
            help="Parquet read buffer size in MiB (streaming mode). Larger values (e.g., 128) reduce request overhead but use more memory.",
        )
        self.parser.add_argument(
        ...
        self.parser.add_argument(
            "--training.enable_expert_parallel",
            action="store_true",
            help="Whether to enable expert parallelization for MoE layers. Now by default, experts are evenly split across replicate.",
        )
        self.parser.add_argument(
