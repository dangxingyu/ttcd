...
def parallelize_fla(
    ...
    if job_config.training.enable_expert_parallel:
        if world_mesh.ndim > 1:
            raise RuntimeError("EP has not supported > 1D parallelism")
        apply_ep(
            model,
            world_mesh
        )
...
TP_PLAN_MAP = {
    ...
    "gla": GLATPPlan,
    "ipttt": TransformerTPPlan,
    "ipttcd": TransformerTPPlan,
}
...
def apply_ep(
    ...
    ep_group = world_mesh.get_group()
    num_moe = 0
    for module in model.modules():
        init_ep = getattr(module, "init_expert_parallel", None)
        if callable(init_ep):
            init_ep(ep_group)
            num_moe += 1
    if num_moe > 0:
        logger.info(
            f"Initialized expert-parallel for {num_moe} MoE module(s) "
            f"with EP group size = {torch.distributed.get_world_size(ep_group)}"
        )
