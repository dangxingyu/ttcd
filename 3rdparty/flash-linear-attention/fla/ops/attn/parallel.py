...
def parallel_attn_fwd_kernel(
    ...
    if USE_L2:
        b_acc2 = tl.zeros([BT], dtype=tl.float32)
        b_acc = None
    else:
    ...
    for i_s in range(0, i_t * BT, BS):
        ...
        if USE_L2:
            ...
            b_acc2 = b_acc2 * b_r2 + tl.sum(b_p * b_p, 1)
...
def parallel_attn_bwd_kernel_dq(
    ...
    for i_s in range(0, i_t * BT, BS):
        ...
        if USE_L2:
            b_ds = b_p * (b_dp.to(tl.float32) - b_p * b_delta[:, None])
        else:
...
def parallel_attn_bwd_kernel_dkv(
    ...
    for i_s in range(i_t * BT, min((i_t + 1) * BT, T), BS):
        ...
        if USE_L2:
            b_ds = b_p * (b_dp - b_p * b_delta[None, :])
        else:
...
class ParallelAttentionFunction(torch.autograd.Function):
    ...
    @contiguous
    @autocast_custom_fwd
    def forward(ctx, q, k, v, g, scale, cu_seqlens, use_l2: bool = False):
        ctx.dtype = q.dtype
...
def parallel_attn(
    ...
    o = ParallelAttentionFunction.apply(q, k, v, g, scale, cu_seqlens, use_l2)
    return o
