...
from typing import Callable, Optional, Tuple, Union, Any, Dict, List, TypedDict
...
class TTTDynamicCache(DynamicCache):
    """Dynamic cache that also stores TTT states per layer.

    Mirrors hf_TTT_qwen3 implementation to persist partial chunk states and
    adapted weights across incremental generation.
    """

    def __init__(self, _distributed_cache_data: Optional[List] = None) -> None:
        super().__init__(_distributed_cache_data)
        # (past_h, past_t, past_w) per layer; default to a large upper bound
        self.ttt_states = [(None, None, None)] * 100

    def TTT_update(
        self, ttt_state: Tuple[torch.Tensor, torch.Tensor, torch.Tensor], layer_idx: int
    ) -> None:
...
class LlamaMLP(nn.Module):
    ...
    def forward(
        ...
        """Forward with optional TTT updates.
        - If `t` is None, acts as standard MLP and returns (output, present_w),
          where present_w equals cloned current down_proj weight or `past_w` if provided.
        - If `t` provided and sequence length >= chunk, performs chunked TTT weight
          updates similar to hf_TTT_qwen3 and returns (output, updated_w).
        """
        h = self.act_fn(self.gate_proj(x)) * self.up_proj(x)
        # Determine current down_proj weight baseline
        present_down_proj_w = (
            self.down_proj.weight.clone() if past_w is None else past_w
        )
        if t is None or not hasattr(self, "ttt_conv"):
            return nn.functional.linear(h, present_down_proj_w), present_down_proj_w

        bs, seq_len, _ = x.shape
        if seq_len < self.ttt_chunk:
            # Not enough tokens to trigger TTT update yet; use current weight
            return nn.functional.linear(h, present_down_proj_w), present_down_proj_w
        ...
        t_conv = (
            self.ttt_conv(
                t_padded.transpose(-1, -2).reshape(bs * chunk_num, -1, chunk_size)
            )
            .transpose(-1, -2)
        ...
        # Iterate over chunks, update outputs and weight online
        for i, current_y, current_t, current_h in zip(
            range(chunk_num), y[0], t_conv[0], h_padded[0]
        ):
            current_y = contract("d h, c h -> c d", current_w, current_h)
            y[0][i] = current_y
            # Update weight except for last partial chunk
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if target_states is None:
            hidden_states, _ = self.mlp(hidden_states, None, None)
        else:
            # Retrieve previous TTT states
            past_h, past_t, past_w = (
                past_key_value.ttt_states[self.layer_idx]
                if past_key_value is not None
                else (None, None, None)
            )
            if past_h is None:
            ...
            if past_key_value is not None:
                past_key_value.TTT_update(
                    (present_h_tail, present_t_tail, present_w), self.layer_idx
                )
        hidden_states = residual + hidden_states
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        if use_cache and past_key_values is None:
            # Use TTT-aware cache to persist custom states across generation
            past_key_values = TTTDynamicCache()
        ...
        for decoder_layer in self.layers[: self.config.num_hidden_layers]:
            ...
            # Follow hf_TTT_qwen3: use the input embeddings as target signals
            target_states = hidden_states

            if self.gradient_checkpointing and self.training:
                layer_outputs = self._gradient_checkpointing_func(
                    partial(decoder_layer.__call__, **flash_attn_kwargs).func,
                    hidden_states,
                    causal_mask,
                    position_ids,
                    past_key_values,
                    ...
                    position_embeddings,
                    target_states
                    if self.ttt_mode and decoder_layer.layer_idx in self.ttt_layers
                    else None,
                )
            else:
...
class LlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
    ...
    def generate(self, **kwargs):
        # Ensure we are using a TTT-aware cache object during generation
        ...
        # The current TTT path supports bs=1 during online updates
